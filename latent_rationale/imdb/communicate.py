...
from latent_rationale.imdb.util import get_predict_args, imdb_reader, \
...
from latent_rationale.imdb.evaluate import evaluate, evaluate_comm
...
def communicate():
    ...
    test_data = list(imdb_reader("data_spec/corpus/imdb/test/data.txt"))
    human_data = list(imdb_reader("data_spec/human-corpus/imdb.txt"))
            ...
            if iter_i == num_iterations:
                ...
                if comm_cfg["save_explanations"]:
                    ...
                    expl_dataset(classifier, layperson, test_data, batch_size=batch_size,
                                 device=device, save_path=comm_cfg["save_path"], ext='test')

                    expl_dataset(classifier, layperson, dev_data, batch_size=batch_size,
                                 device=device, save_path=comm_cfg["save_path"], ext='dev')

                    expl_dataset(classifier, layperson, human_data, batch_size=batch_size,
                                 device=device, save_path=comm_cfg["save_path"], ext='human')
