...
from latent_rationale.yelp.constants import UNK_TOKEN, PAD_TOKEN
from latent_rationale.yelp.plotting import plot_heatmap
...
def yelp_reader(path, lower=False):
    """
    Reads in examples
    :param path:
    :param lower:
    ...
    """
    # labels = set()
    # minlen, maxlen = 10000000, 0
    tokenizer = nltk.WordPunctTokenizer()
    for line in filereader(path):
        data = json.loads(line.strip())
        label = int(data['stars']) - 1
        # labels.add(label)
        text = data['text'].lower() if lower else data['text']
        tokens = tokenizer.tokenize(text)
        # minlen = min(len(tokens), minlen)
...
def get_predict_args():
    parser = argparse.ArgumentParser(description='Yelp prediction')
...
def get_args():
    ...
    parser.add_argument('--save_path', type=str, default='yelp_results/default')
    ...
    parser.add_argument('--word_vectors', type=str,
                        default='data_spec/embs/glove/glove.840B.300d.small.raw.pickle.yelp')
    args = parser.parse_args()
    return args
...
def get_comm_args():
    ...
    parser.add_argument('--save_path', type=str, default='yelp_comm_results/default')
    parser.add_argument('--resume_snapshot', type=str, default='')
